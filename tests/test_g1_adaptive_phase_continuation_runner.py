...
class G1AdaptivePhaseContinuationRunnerTest(unittest.TestCase):
    def test_contract_is_exact_e006_continuation_plus_adaptive_treatment(self):
        from tools.run_canonical_g1_shac import build_canonical_kwargs
        from tools.run_g1_adaptive_phase_continuation import (
            build_adaptive_phase_continuation_kwargs,
        )
        ...
        candidate = build_adaptive_phase_continuation_kwargs(
            "g1-4x5", reference, seed=0, resume_from=checkpoint
        )
    ...
    def test_parser_requires_resume_and_rejects_scientific_overrides(self):
        from tools.run_g1_adaptive_phase_continuation import build_parser
        ...
        for override in (
            ...
            ["--unroll-length", "24"],
            ["--actor-lr", "0.001"],
            ["--adaptive-phase-uniform-ratio", "0.75"],
            ["--adaptive-phase-alpha", "0.01"],
            ["--adaptive-phase-sampling", "false"],
        ):
            arguments = [
                "--solver-profile",
                "g1-4x5",
