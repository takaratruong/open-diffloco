...
class ShacExactResumeTest(unittest.TestCase):
    def test_root_velocity_weight_resume_is_explicit_and_fail_closed(self):
        ...
        self.assertEqual(
            resolve_tracking_root_velocity_resume_weight(
                None, requested=1.0, allow_change=False, is_resume=False
            ),
            1.0,
        )
        self.assertEqual(
            resolve_tracking_root_velocity_resume_weight(
                {}, requested=0.0, allow_change=False, is_resume=True
            ),
            0.0,
        )
        with self.assertRaisesRegex(ValueError, "hparams"):
            resolve_tracking_root_velocity_resume_weight(
                None, requested=0.0, allow_change=False, is_resume=True
            )
    ...
    def test_root_velocity_train_settings_are_default_off(self):
        from src.algorithms.shac.algorithm import train

        parameters = inspect.signature(train).parameters

        self.assertEqual(parameters["tracking_root_velocity_weight"].default, 0.0)
        self.assertIs(
            parameters["allow_resume_tracking_root_velocity_change"].default,
            False,
        )
