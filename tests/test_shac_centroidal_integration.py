...
def test_centroidal_and_frozen_controller_flags_are_default_off() -> None:
    ...
    assert (
        parameters["allow_resume_actor_capture_point_tracking_start"].default
        is False
    )
...
def test_capture_point_upgrade_requires_explicit_resume_authority() -> None:
    ...
    legacy = {"actor_capture_point_tracking": False}
    with pytest.raises(ValueError, match="explicit authority"):
        resolve_capture_point_tracking_resume_settings(
            legacy,
            requested_enabled=True,
    ...
    assert resolve_capture_point_tracking_resume_settings(
        legacy,
        requested_enabled=True,
        requested_delta=0.1,
        requested_weight=0.25,
...
def test_capture_point_treatment_rejects_assistance_and_unfrozen_parent() -> None:
    from src.algorithms.shac.algorithm import (
    ...
    validate_capture_point_tracking_configuration(**valid)
    for change in (
        {"frozen_controller_residual": False},
        {"torso_wrench_assistance": True},
        {"actor_learned_torso_wrench": True},
