"""Continue E006 with immutable failure-adaptive reference-phase resets."""
...
def build_adaptive_phase_continuation_kwargs(
    profile_name: str,
    reference_path: str | Path,
    seed: int,
    resume_from: str | Path,
) -> dict:
    """Build the fixed adaptive continuation contract from canonical G1."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Immutable failure-adaptive continuation of E006."
    )
    parser.add_argument(
        "--solver-profile",
        required=True,
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("g1_adaptive_phase_continuation_runs"),
    )
...
def main() -> None:
    ...
    kwargs = build_adaptive_phase_continuation_kwargs(
        args.solver_profile,
