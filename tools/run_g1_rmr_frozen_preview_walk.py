"""Train only new RMR preview columns on the default walking reference."""
...
def build_rmr_frozen_preview_walk_kwargs(
    profile_name: str,
    reference_path: str | Path,
    seed: int,
    source_actor,
) -> dict:
    """Build the immutable one-frame native-RMR preview treatment."""
    kwargs = build_canonical_kwargs(
        profile_name, reference_path, seed, resume_from=None
    )
    kwargs.update(
        total_steps=393_216,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("g1_rmr_frozen_preview_walk_runs"),
    )
    return parser
...
def main() -> None:
    ...
    kwargs = build_rmr_frozen_preview_walk_kwargs(
        args.solver_profile,
