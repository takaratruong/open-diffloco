"""Repeat frozen nonlinear preview training with Muon on its input matrix."""
...
def build_frozen_residual_muon_kwargs(
    profile_name: str,
    reference_path: str | Path,
    seed: int,
    resume_from: str | Path,
) -> dict:
    """Change only the frozen residual adapter optimizer to Muon."""
    ...
    kwargs["actor_residual_preview_optimizer"] = "muon"
    return kwargs
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("g1_frozen_residual_muon_runs"),
    )
...
def main() -> None:
    ...
    kwargs = build_frozen_residual_muon_kwargs(
        args.solver_profile,
