"""Continue exact E012 selected policy with actor bootstrap disabled."""
...
def expected_checkpoint_steps() -> tuple[int, ...]:
    ...
    return tuple(
        range(
            E012_SELECTED_STEP + CHECKPOINT_INTERVAL,
            ZERO_BOOTSTRAP_END_STEP + 1,
            CHECKPOINT_INTERVAL,
        )
    )


def build_zero_bootstrap_kwargs(
    profile_name: str,
    reference_path: str | Path,
    seed: int,
    resume_from: str | Path,
) -> dict:
    """Change only endpoint and explicit actor-bootstrap treatment fields."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("g1_zero_bootstrap_continuation_runs"),
    )
...
def validate_preflight(
    *,
    ...
    """Fail closed on the exact E012 selected starting boundary."""
    ...
    return {
        "protocol": "g1-zero-bootstrap-preflight-v1",
        "code_commit": head,
...
def validate_training_artifacts(run_directory: Path) -> dict[str, Any]:
    """Validate the exact zero-bootstrap dense treatment artifacts."""
    ...
    return {
        "protocol": "g1-zero-bootstrap-training-v1",
        "valid": True,
...
def main() -> None:
    ...
    _write_json_atomically(output_root / "zero_bootstrap_preflight.json", preflight)
    configure_jax()
    ...
    _write_json_atomically(
        output_root / "zero_bootstrap_training_validation.json", validation
    )
    print(run_directory)
